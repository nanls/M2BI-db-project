...
class UploadForm(FlaskForm):
    ...
    angle_unit = RadioField(
        'Angle units',
        choices=[('degree', 'degree (default)'), ('radian', 'radian')]
    )
...
class SearchByPDBidForm(FlaskForm):
    ...
    PDBid = TextAreaField(
        'PDB ID or list of PDB IDs separated by space or carriage return'
    )
    submit = SubmitField(
        'Search',
        render_kw={"class": "btn btn-info btn-lg", "id": "submit-button"}
...
class SearchFilesForm(FlaskForm):
    """
    Form for display PDB in database according to resolution and/or criteria.
    """

    # Resolution?
    resMin = DecimalField(
        'Minimum resolution',
        places=2, rounding=None, validators=[optional()]
    )
    resMax = DecimalField(
        'Maximum resolution',
        places=2, rounding=None, validators=[optional()]
    ...
    submit = SubmitField(
        'Display list of PDB id in database',
        render_kw={"class": "btn btn-info btn-lg", "id": "submit-button"}
    )
